...
//    вобьем вручную
        ...
        System.out.println(endOfTomorrowClass);
...
//     V. Как парсить и как форматировать
        //если строка приходит через дефис - 2022-12-12
        // парсинг
        LocalDate christmas = LocalDate.parse("2023-12-24");
        System.out.println(christmas);
        //если формат передачи другой будет ошибка, поэтому используем форматтер
        DateTimeFormatter formatter = DateTimeFormatter.ofPattern("yyyy/MM/dd");
        LocalDate formatting = LocalDate.parse("2023/12/24", formatter);

        //Format

        String unityStile = formatter.format(unity);//значение unity берем из строки 72
        System.out.println(unityStile);
